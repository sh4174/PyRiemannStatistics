...
regExePath = 'python /media/shong/IntHard1/Projects/4DShapeAnalysis/Code/pydeformetrica/deformetrica/src/deformetrica.py estimate -v DEBUG'
...
outFolderPath = '/media/shong/IntHard1/4DAnalysis/IPMI2019/GeodesicRegressionResults/Surface_Complex_CTRL_Deformetrica/output'
baselineFolderPath = '/media/shong/IntHard1/4DAnalysis/IPMI2019/GeodesicRegressionResults/Surface_Complex_CTRL_Deformetrica/output_atlas/' 

paramModelPath = folderPath + 'model.xml'
paramDataSetPath = folderPath + 'data_set.xml'
paramOptPath = folderPath + 'optimization_parameters.xml' 
...
model_f.write( "  <model-type>Regression</model-type>\n" )
...
for p in range( len( anatomy_list ) ):
	...
	initTemplateAnatomyPath = baselineFolderPath + 'DeterministicAtlas__EstimatedParameters__Template_' + anatomy_list[ p ] + '.vtk'
...
opt_f.write( "    <max-iterations>1000</max-iterations>\n" )
...
dataSet_f.write( "  <subject id=\"Cross-Sectional-Control\">\n" )
...
# for i in range( 10 ):
	...
	for j in range( len( dataInfoList[i].LabelList ) ):
		...
		for anatomy in anatomy_list:
			subj_i_label_j_folderPath = dataFolderPath + 'PHD-AS1-' + dataInfoList[i].ID + "/" + dataInfoList[i].LabelList[j ] + "/surfaces/decimated_aligned/cmrep_"

			# print( "ID : " + dataInfoList[i].ID + ", Label : " + dataInfoList[i].LabelList[j ] )

			input_path = subj_i_label_j_folderPath + anatomy + "/mesh/def3_target.vtk"
		...
		dataSet_f.write( "    <visit id=\"" + dataInfoList[i].ID + "\">\n" )
