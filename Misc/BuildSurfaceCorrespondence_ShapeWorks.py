...
for anatomy in anatomy_list:
	...
	# for i in range( 10 ):
		...
		for j in range( len( dataInfoList[i].LabelList ) ):
			...
			corr_output_vtk_path = subj_i_label_j_folderPath + anatomy + "_corr_p850_pts.vtk"


			vtk_input_path = subj_i_label_j_folderPath + anatomy + ".vtk" 
			...
			corr_output_vtk_arr.append( corr_output_vtk_path )
			vtk_input_arr.append( vtk_input_path ) 
	...
	param_pre2_file.write( '<antialias_iterations> 10 </antialias_iterations>\n' )
	param_pre2_file.write( ' \n' )
	param_pre2_file.write( '<!-- Size of Gaussian blurring kernel for smoothing -->\n' )
	param_pre2_file.write( '<blur_sigma> 0.05 </blur_sigma>\n' )
	param_pre2_file.write( ' \n' )
	param_pre2_file.write( ' \n' )
	...
	param_corr_file.write( '<number_of_particles> 850 </number_of_particles>\n' )
	param_corr_file.write( '\n' )
	...
	param_corr_file.write( '<!-- Prefix for the output files. -->\n' )
	param_corr_file.write( '<output_points_prefix>' + anatomy + '</output_points_prefix>\n')
	...
	for k in range( len( corr_output_arr ) ):
		os.system( 'mv ' + anatomy + '.' + str( k ).zfill(3) + '.wpts ' + corr_output_arr[ k ] )

	# WPTS to VTK
	for k in range( len( corr_output_arr ) ):
		# VTK Input File
		surf_data_reader = vtk.vtkPolyDataReader()
		surf_data_reader.SetFileName( vtk_input_arr[ k ] )
		surf_data_reader.Update()

		surf_data = surf_data_reader.GetOutput()

		centerFilter = vtk.vtkCenterOfMass()
		centerFilter.SetInputData( surf_data )
		...
		pt_mat = np.loadtxt( corr_output_arr[ k ] )

		print( pt_mat.shape )
		points = vtk.vtkPoints()
	
		for p in range( pt_mat.shape[ 0 ] ):
			points.InsertNextPoint( pt_mat[ p, 0 ], pt_mat[ p, 1 ], pt_mat[ p, 2 ] )

		polyData = vtk.vtkPolyData()
		polyData.SetPoints( points )

		vertFilter = vtk.vtkVertexGlyphFilter()
		vertFilter.SetInputData( polyData )
		vertFilter.Update()

		centerVertFilter = vtk.vtkCenterOfMass()
		centerVertFilter.SetInputData( vertFilter.GetOutput() )
		centerVertFilter.SetUseScalarsAsWeights( False )
		centerVertFilter.Update()

		centerVert = centerVertFilter.GetCenter()

		# Correspondence Center Matched
		points2 = vtk.vtkPoints()
	
		for p in range( pt_mat.shape[ 0 ] ):
			points2.InsertNextPoint( pt_mat[ p, 0 ] + center[ 0 ] - centerVert[ 0 ], pt_mat[ p, 1 ] + center[ 1 ] - centerVert[ 1 ], pt_mat[ p, 2 ] + center[ 2 ] - centerVert[ 2 ] )

		polyData2 = vtk.vtkPolyData()
		polyData2.SetPoints( points2 )

		vertFilter2 = vtk.vtkVertexGlyphFilter()
		vertFilter2.SetInputData( polyData2 )
		vertFilter2.Update()

		writer = vtk.vtkPolyDataWriter()
		writer.SetFileName( corr_output_vtk_arr[ k ] )
		writer.SetInputData( vertFilter2.GetOutput() )
		writer.Update()
		writer.Write()
