...
for n in range( nData ):
	if riskGroupList[ n ] == "cont":
		pt_list_cont.append( CMRepDataList[ n ].pt[ k_cmrep ][ 1 ].pt )
		t_list_cont.append( ageList[ n ] )

	if riskGroupList[ n ] == "high":
		pt_list_high.append( CMRepDataList[ n ].pt[ k_cmrep ][ 1 ].pt )
...
for i in range( len( dataInfoList )  ):
	...
	if dataInfoList[i].GenderList[ 0 ] == 'm':
		continue
