...
for n in range( nSet ):
	...
	# Project a tangent vector to a point 
	rand_tVec_proj = p_interp.ProjectTangent( p_interp, rand_tVec )

	# Perturb a point 
	p_interp_perturbed = p_interp.ExponentialMap( rand_tVec_proj )

	pert_pt_list.append( p_interp_perturbed )

	# Parallel transport a slope tangent vector
	v_slope_pert_pt = p_interp.ParallelTranslateAtoB( p_interp, p_interp_perturbed, v_slope )
	pert_v_slope_list.append( v_slope_pert_pt )
