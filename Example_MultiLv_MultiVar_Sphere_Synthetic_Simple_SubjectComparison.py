...
# A tangent vector for CAG repeat
beta1.SetTangentVector( [ 0.0, 0.0, 0.4 ] )
...
gamma0.SetTangentVector( [ 0.0, 0.01, 0.02 ] )

# A slope tangent vector for CAG and age
gamma1.SetTangentVector( [ 0.0, 0.002, -0.003 ] )

# gamma1.SetTangentVector( [ 0.0, 0.002, -0.002 ] )
...
for c in range( nCAG_Lines ):
	...
	beta1_c = beta1.ScalarMultiply( c_pt )

	p0_c = beta0.ExponentialMap( beta1_c )

	gamma0_tilde = beta0.ParallelTranslateAtoB( beta0, p0_c, gamma0 )
	gamma1_tilde = beta0.ParallelTranslateAtoB( beta0, p0_c, gamma1 )
...
for i in range( nSubject ):
	...
	cov_int_list.append( [ c_i ] )
	cov_slope_list.append( [ c_i ] ) 

	cov_int_list_null.append( [0] )
	cov_slope_list_null.append( [0] ) 
...
est_gamma0.SetTangentVector( tangent_slope_arr[ 1 ].tVector )
...
for c in range( nCAG_Lines ):
	...
	beta2_c = est_beta1.ScalarMultiply( c_pt )

	p0_c = est_beta0.ExponentialMap( beta2_c )

	gamma0_tilde = est_beta0.ParallelTranslateAtoB( est_beta0, p0_c, est_gamma0 )
	gamma1_tilde = est_beta0.ParallelTranslateAtoB( est_beta0, p0_c, est_gamma1 )
...
est_gamma0_null.SetTangentVector( tangent_slope_arr_null[ 1 ].tVector )
...
for c in range( nCAG_Lines ):
	...
	beta1_c = est_beta1_null.ScalarMultiply( c_pt )

	p0_c = est_beta0_null.ExponentialMap( beta1_c )

	gamma0_tilde = est_beta0.ParallelTranslateAtoB( est_beta0_null, p0_c, est_gamma0_null )
	gamma1_tilde = est_beta0.ParallelTranslateAtoB( est_beta0, p0_c, est_gamma1_null )
...
ind_est_group_vtk_list_HMG = [] 
ind_est_group_vtk_list_HSG = [] 
ind_est_group_vtk_list_SG = []
...
for g in range( len( pt_list ) ):
	...
	c_g = cov_int_list[ g ][ 0 ]	

	for t in range( nLineTimePt ):
		time_pt = ( t_1_g - t_0_g ) * t / nLineTimePt + t_0_g

		v_t_SG = manifolds.sphere_tVec( nDimManifold )
		v_t_SG.SetTangentVector( [ v_group_list[ g ].tVector[ 0 ] * time_pt, v_group_list[ g ].tVector[ 1 ] * time_pt, v_group_list[ g ].tVector[ 2 ] * time_pt ] )
...
for n in range( len( pt_list_all ) ):
	...
	for k in range( nDimManifold ):
		beta_n.tVector[ k ] = est_beta1.tVector[ k ] * c_n
	...
	for k in range( nDimManifold ):
		gamma_n.tVector[ k ] = ( gamma0_at_p_1_n.tVector[ k ] + gamma1_at_p_1_n.tVector[ k ] * c_n ) * t_n 
...
for n in range( len( pt_list_all ) ):
	...
	for k in range( nDimManifold ):
		beta_n.tVector[ k ] = est_beta1_null.tVector[ k ] * c_n
...
for i in range( len( pt_list ) ):
	...
	c_i = cov_int_list[ i ][ 0 ]

	beta_i = manifolds.sphere_tVec( nDimManifold )

	for j in range( nDimManifold ):
		beta_i.tVector[ j ] = est_beta1.tVector[ j ] * c_i
...
for i in range( len( subj_slope_list ) ):
	...
	for j in range( nDimManifold ):
		gamma_i.tVector[ j ] = ( est_gamma0.tVector[ j ] + est_gamma1.tVector[ j ] * c_i ) 
...
estGroupGeodesicMapper_null.SetInputData( est_group_vtk_list_s1_null[ 0 ] )
...
for i in range( len( pt_list_all ) ):
	...
	color_i_s_c = [ 0, int( 255.0 * float( c_i - c0 ) / float( c1 - c0 ) ), int( 255.0 * float( c1 - c_i ) / float( c1 - c0 ) ) ] 
...
for j in range( len( group_vtk_list_s1 ) ):
	c_ij = group_c_list_s1[ j ]

	# color_ij = [ 0.0, ( 1.0 * float( c_ij - c0 ) / float( c1 - c0 ) ), 1.0 ]  
	color_ij = [ 0, 1.0 * float( c_ij - c0 ) / float( c1 - c0 ), 1.0 * float( c1 - c_ij ) / float( c1 - c0 ) ] 


	group_geodesic_mapper = vtk.vtkPolyDataMapper()
	group_geodesic_mapper.SetInputData( group_vtk_list_s1[ j ] )
...
for j in range( len( est_group_vtk_list_s1 ) ):
	c_ij = group_c_list_s1[ j ]

	color_ij = [ 0, 1.0 * float( c_ij - c0 ) / float( c1 - c0 ), 1.0 * float( c1 - c_ij ) / float( c1 - c0 ) ] 

	estGroupGeodesicMapper = vtk.vtkPolyDataMapper()
	estGroupGeodesicMapper.SetInputData( est_group_vtk_list_s1[ j ] )
