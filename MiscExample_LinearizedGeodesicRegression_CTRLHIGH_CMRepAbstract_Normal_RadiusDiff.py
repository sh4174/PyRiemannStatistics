...
for a, anatomy in enumerate( anatomy_list ):
	...
	# Radius Change
	radius_change_folder_path = '/media/shong/IntHard1/4DAnalysis/IPMI2019/GeodesicRegressionResults/CMRep_Abstract_Normal_CTRLHIGH_Group_NewCode_Linearized_Complex/RadDiff/'

	for n in range( nTimePt ):
		outFileName = 'CMRep_Abstract_Normal_Regression_CTRLHIGH_' + anatomy + '_Radius_Diff_' + str( n ) + '.vtk' 
		...
		for k in range( nManDim ):
			...
			polyData_t.GetPointData().GetArray( "Radius" ).SetValue( k, ( est_cmrep_HR_t.pt[3].pt[ k ] - est_cmrep_t.pt[ 3 ].pt[ k ] ) )
