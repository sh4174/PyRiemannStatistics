...
for s in range( 2 ):
	for aut in range( 2 ):
		for ados in range( 11 ):
			...
			for k in range( 2 ):
				for j in range( nPt ):
					beta_i.tVector[ k, j ] = ( est_beta1.tVector[ k, j ] * s + est_beta2.tVector[ k, j ] * aut + est_beta3.tVector[ k, j ] * ados )
			...
			for k in range( 2 ):
				for j in range( nPt ):
					gamma_i.tVector[ k, j ] = ( gamma0_p1_i.tVector[ k, j ] + gamma1_p1_i.tVector[ k, j ] * s + gamma2_p1_i.tVector[ k, j ] * aut + gamma3_p1_i.tVector[ k, j ] * ados )
			...
			plt.savefig( '/media/shong/IntHard1/Projects/4DShapeAnalysis/Data/ACE_IBIS_CC/Data/MLMG_LRHR_Autism_ADOS/results_s' + str( s ) + 'aut' + str( aut ) + 'ADOS' + str( ados ) + '.png' )
			plt.close()
	...
	for s in range( 2 ):
		for aut in range( 2 ):
			...
			for ados in range( 11 ):
				...
				plt.plot( x_t, y_t, color=colors_2[ ados ].rgb, alpha=0.6, label=("s" + str( s ) + "aut" + str( aut ) + "ADOS" + str( ados ) ) )
				# plt.legend()

			plt.savefig( '/media/shong/IntHard1/Projects/4DShapeAnalysis/Data/ACE_IBIS_CC/Data/MLMG_LRHR_Autism_ADOS/results_' + 's'+ str( s ) + 'aut' + str( aut ) + 't' + str( i_t ) + '.png' )
			plt.close()
...
for i in range( len( shape_age_list_a ) ):
	...
	for j in range( len( shape_list_i ) ):
		...
		plt.savefig( '/media/shong/IntHard1/Projects/4DShapeAnalysis/Data/ACE_IBIS_CC/Data/MLMG_LRHR_Autism_ADOS/Subjects/' + subj_ij + "_" + visit_label_ij + "_s" + str( s_ij ) + "aut" +  str( autism_ij ) + "ADOS" + str( ados_ij ) + '.png' )
		plt.close()
